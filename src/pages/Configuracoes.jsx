    ...
    <div className="space-y-6 max-w-3xl">
      ...
      <Tabs defaultValue="regras">
        <TabsList className="rounded-xl mb-2">
          <TabsTrigger value="regras" className="rounded-lg gap-1.5"><Shield className="w-3.5 h-3.5" />Regras</TabsTrigger>
          <TabsTrigger value="ixc" className="rounded-lg gap-1.5"><Plug className="w-3.5 h-3.5" />IXC</TabsTrigger>
          <TabsTrigger value="sincronizar" className="rounded-lg gap-1.5"><RefreshCw className="w-3.5 h-3.5" />Sincronizar</TabsTrigger>
        </TabsList>

        {/* ── Regras de Negócio ── */}
        <TabsContent value="regras" className="space-y-5 mt-4">
          <Card className="rounded-2xl">
            <CardHeader>
              <CardTitle className="flex items-center gap-2"><Shield className="w-5 h-5 text-primary" /> Regras de Crédito</CardTitle>
              <CardDescription>Critérios de aprovação automática</CardDescription>
            </CardHeader>
          ...
          <div className="flex justify-end">
            <Button onClick={() => saveMutation.mutate(form)} disabled={saveMutation.isPending} className="gap-2 rounded-xl px-6">
        ...
        {/* ── IXC Manual ── */}
        <TabsContent value="ixc" className="mt-4">
          <Card className="rounded-2xl">
            <CardHeader>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5"><Label>ID Filial</Label><Input value={form.id_filial_ixc} onChange={e => set("id_filial_ixc", e.target.value)} className="rounded-xl" /></div>
                <div className="space-y-1.5"><Label>ID Vendedor Padrão</Label><Input value={form.id_vendedor_ixc_padrao} onChange={e => set("id_vendedor_ixc_padrao", e.target.value)} className="rounded-xl" /></div>
                <div className="space-y-1.5"><Label>ID Assunto OS (Instalação)</Label><Input value={form.id_assunto_os_ixc} onChange={e => set("id_assunto_os_ixc", e.target.value)} className="rounded-xl" /></div>
                <div className="space-y-1.5"><Label>ID Setor OS</Label><Input value={form.id_setor_os_ixc} onChange={e => set("id_setor_os_ixc", e.target.value)} className="rounded-xl" /></div>
                <div className="space-y-1.5"><Label>Contribuinte PF</Label><Input value={form.contribuinte_pf} onChange={e => set("contribuinte_pf", e.target.value)} className="rounded-xl" /></div>
                <div className="space-y-1.5"><Label>Status Contrato Inicial</Label><Input value={form.status_contrato_inicial} onChange={e => set("status_contrato_inicial", e.target.value)} className="rounded-xl" /></div>
              </div>
              <div className="flex justify-end pt-2">
                <Button onClick={() => saveMutation.mutate(form)} disabled={saveMutation.isPending} className="gap-2 rounded-xl px-6">
                  <Save className="w-4 h-4" />
                  {saveMutation.isPending ? "Salvando..." : "Salvar IXC"}
                </Button>
              </div>
            </CardContent>
          </Card>
