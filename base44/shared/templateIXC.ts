// base44/shared/templateIXC.ts
// Módulo compartilhado para preencher modelos de contrato do IXC.

export const onlyDigits = (v: string) => (v ? String(v).replace(/\D/g, "") : "");

export const fmtDoc = (v: string) => {
...
export function dataExtenso(date: Date | string = new Date()) {
...
export function montarVariaveisIXC(ctx: any = {}) {
...
export function aplicarVariaveis(conteudo: string, vars: Record<string, any>) {
  if (!conteudo) return "";
  const get = (k: string) => {
...
export function renderizarContrato(modelo: string, ctx: any = {}) {
